...
@mcp.tool()
async def chat_stream(question: str, token: str = "", use_web_search: bool = False, use_hybrid: bool = False) -> list[TextContent]:
    """
    向 RAG 系统提问并获取流式答案（逐字返回，支持实时打字效果）。
    ...
    返回值:
        str: 包含完整答案和来源列表的响应（流式接收后汇总）。
    
    示例用法:
        # 知识库检索
        chat_stream(question="什么是RAG？", token="your_token")
        
        # 互联网搜索
        chat_stream(question="Python最新版本是什么？", token="your_token", use_web_search=True)
        
        # 混合搜索
        chat_stream(question="RAG技术的最新发展", token="your_token", use_hybrid=True)
    ...
    注意事项:
        ...
        - 此接口使用 SSE（Server-Sent Events）流式传输，适合需要实时展示的场景
    """
    # 获取请求对象
    ...
    try:
        ...
        if response.status_code != 200:
            if response.status_code == 401:
                ...
                return [TextContent(type="text", text=f"流式问答请求失败，状态码: {response.status_code}，信息: {response.text}")]
            ...
            if line:
                ...
                if line.startswith('data: '):
                    try:
                        ...
                        # 处理错误
                        if 'error' in data:
                            return [TextContent(type="text", text=f"生成答案时发生错误: {data['error']}")]
                    except json.JSONDecodeError:
                        continue
        
        # 格式化最终结果
        result_text = f"【答案】\n{full_answer or '未生成答案'}\n\n"
